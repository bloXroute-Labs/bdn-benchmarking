...
async def main() -> None:
    ...
    if "MISSING" in args.dump:
        missing_hashes_file = cast(
            TextIO, open("missing_block_hashes.txt", "w"))
    else:
...
async def process_new_blocks_bdn(feed_name: str, exclude_block_contents: bool):
    ...
    while True:
        ...
        if block_hash in seen_hashes:
            ...
            seen_hashes[block_hash] = HashEntry(
                block_hash, gateway_time_received=time.time())
        else:
...
def handle_eth_block_hash(block_hash, time_received: float):
    if block_hash in seen_hashes:
        ...
        seen_hashes[block_hash] = HashEntry(
            block_hash, node_time_received=time_received)
    else:
        trail_new_hashes.add(block_hash)
...
async def fetch_block(block_hash: str) -> None:
    try:
        ...
        print(
            f"Attempt to fetch full block for {block_hash} failed. Abandoning.")
        pass
...
def stats(ignore_delta: int) -> str:
    ...
    for block_hash, hash_entry in seen_hashes.items():
        if hash_entry.gateway_time_received is None:
            ...
            if all_hashes_file:
                all_hashes_file.write(
                    f"{block_hash}, 0, {datetime.fromtimestamp(eth_node_time_received)}\n")
        ...
        if hash_entry.eth_node_time_received is None:
            ...
            if all_hashes_file:
                all_hashes_file.write(
                    f"{block_hash}, {datetime.fromtimestamp(gateway_time_received)}, 0\n")
        ...
        if gateway_time_received < eth_node_time_received:
            ...
            block_received_by_gateway_first_total_delta += eth_node_time_received - \
                gateway_time_received
        elif eth_node_time_received < gateway_time_received:
            new_blocks_from_eth_node_feed_first += 1
            blocks_seen_by_both_feeds_eth_node_first += 1
            block_received_by_eth_node_first_total_delta += gateway_time_received - \
                eth_node_time_received

    new_block_seen_by_both_feeds = blocks_seen_by_both_feeds_gateway_first + \
        blocks_seen_by_both_feeds_eth_node_first
...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--feed-name", type=str,
                        default="bdnBlocks", choices=["newBlocks", "bdnBlocks"])
    ...
    parser.add_argument("--trail-time", type=int, default=60,
                        help="Seconds to wait after interval to receive block on both feeds")
    ...
    parser.add_argument("--ignore-delta", type=int, default=5,
                        help="Ignore block with delta above this amount (seconds)")
