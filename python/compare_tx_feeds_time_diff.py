...
async def main() -> None:
    ...
    min_gas = None if not args.min_gas_price else int(
        args.min_gas_price * 10e8)
    ...
    while True:
        ...
        if num_intervals_passed == args.num_intervals:
            print(
                f"{num_intervals_passed} of {num_intervals} intervals complete. Exiting.")
            sys.exit(0)
...
async def process_new_txs_bdn(
    ...
    while True:
        ...
        if transaction_hash in seen_hashes:
            ...
            seen_hashes[transaction_hash] = HashEntry(
                transaction_hash, gateway_time_received=time.time())
        else:
...
async def process_new_txs_eth(
    ...
    async with EthWsSubscriber(eth_url) as eth_ws:
        ...
        while True:
            ...
            if not exclude_tx_contents:
                asyncio.create_task(fetch_transaction_contents(
                    transaction_hash, min_gas, addresses))
                ...
                seen_hashes[transaction_hash] = HashEntry(
                    transaction_hash, node_time_received=time_received)
            else:
                trail_new_hashes.add(transaction_hash)
...
async def fetch_transaction_contents(tx_hash: str, min_gas: Optional[int], addresses: List[str]) -> None:
    try:
        ...
        if tx_hash in seen_hashes:
            ...
            seen_hashes[tx_hash] = HashEntry(
                tx_hash, node_time_received=time.time())
        elif tx_contents is not None:
            trail_new_hashes.add(tx_hash)
    except Exception:
        print(
            f"Attempt to fetch transaction contents for {tx_hash} failed. Abandoning.")
        pass
    ...
    for tx_hash, hash_entry in seen_hashes.items():
        if hash_entry.gateway_time_received is None:
            ...
            if all_hashes_file:
                ...
                assert f is not None
                f.write(
                    f"{tx_hash}, 0, {datetime.fromtimestamp(eth_node_time_received)}, 0\n")
        ...
        if hash_entry.eth_node_time_received is None:
            ...
            if all_hashes_file:
                ...
                assert f is not None
                f.write(
                    f"{tx_hash}, {datetime.fromtimestamp(gateway_time_received)}, 0, 0\n")
        ...
        if gateway_time_received < eth_node_time_received:
            ...
            tx_received_by_eth_node_first_total_delta += gateway_time_received - \
                eth_node_time_received

    new_tx_seen_by_both_feeds = tx_seen_by_both_feeds_gateway_first + \
        tx_seen_by_both_feeds_eth_node_first
...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--gateway", type=str,
                        default="ws://127.0.0.1:28333/ws")
    parser.add_argument("--eth", type=str,
                        default="ws://127.0.0.1:8546")
    parser.add_argument("--feed-name", type=str, default="newTxs",
                        choices=["newTxs", "pendingTxs", "transactionStatus"])
    parser.add_argument("--min-gas-price", type=float,
                        default=None, help="Gas price in gigawei")
    parser.add_argument("--addresses", type=str, default=None,
                        help="Comma separated list of Ethereum addresses")
    parser.add_argument("--exclude-tx-contents", action="store_true")
    parser.add_argument("--interval", type=int, default=60,
                        help="Length of feed sample interval in seconds")
    parser.add_argument("--num-intervals", type=int, default=1)
    parser.add_argument("--lead-time", type=int, default=60,
                        help="Seconds to wait before starting to compare feeds")
    parser.add_argument("--trail-time", type=int, default=60,
                        help="Seconds to wait after interval to receive tx on both feeds")
    parser.add_argument("--dump", type=str, default="",
                        choices=["ALL", "MISSING", "ALL,MISSING"])
    parser.add_argument("--exclude-duplicates", action="store_true",
                        default=True, help="For pendingTxs only")
    parser.add_argument("--ignore-delta", type=int, default=5,
                        help="Ignore tx with delta above this amount (seconds)")
    ...
    parser.add_argument("--cloud-api-ws-uri", type=str,
                        default="wss://api.blxrbdn.com/ws")
    parser.add_argument("--auth-header", type=str,
                        help="Authorization header created with account id and password")
