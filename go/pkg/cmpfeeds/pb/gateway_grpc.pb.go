...
type GatewayClient interface {
	...
	NewTx(ctx context.Context, in *NewTxRequest, opts ...grpc.CallOption) (Gateway_NewTxClient, error)
}
...
func (c *gatewayClient) NewTx(ctx context.Context, in *NewTxRequest, opts ...grpc.CallOption) (Gateway_NewTxClient, error) {
	stream, err := c.cc.NewStream(ctx, &Gateway_ServiceDesc.Streams[0], "/gateway.Gateway/NewTx", opts...)
	if err != nil {
		return nil, err
	}
	x := &gatewayNewTxClient{stream}
...
type Gateway_NewTxClient interface {
	Recv() (*NewTxReply, error)
	grpc.ClientStream
}

type gatewayNewTxClient struct {
	grpc.ClientStream
}

func (x *gatewayNewTxClient) Recv() (*NewTxReply, error) {
	m := new(NewTxReply)
...
type GatewayServer interface {
	...
	NewTx(*NewTxRequest, Gateway_NewTxServer) error
...
func (UnimplementedGatewayServer) NewTx(*NewTxRequest, Gateway_NewTxServer) error {
	return status.Errorf(codes.Unimplemented, "method NewTx not implemented")
}
...
func _Gateway_NewTx_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(NewTxRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(GatewayServer).NewTx(m, &gatewayNewTxServer{stream})
}

type Gateway_NewTxServer interface {
	Send(*NewTxReply) error
	grpc.ServerStream
}

type gatewayNewTxServer struct {
	grpc.ServerStream
}

func (x *gatewayNewTxServer) Send(m *NewTxReply) error {
...
var Gateway_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "NewTx",
			Handler:       _Gateway_NewTx_Handler,
			ServerStreams: true,
		},
	},
